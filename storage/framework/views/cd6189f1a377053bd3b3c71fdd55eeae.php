            ...
            <table class="w-full border-collapse text-left text-sm text-slate-700">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50/80 transition">
                        ...
                        <td class="p-4 font-mono text-xs text-slate-600"><?php echo e($item->start_date?->format('Y-m-d') ?? '—'); ?></td>
                        <td class="p-4 font-mono text-xs text-slate-600">
                            <?php echo e($item->effective_end_date?->format('Y-m-d') ?? '—'); ?>

                            <?php if($item->extended_end_date): ?>
                                <span class="inline-block ml-1 bg-amber-50 text-amber-700 border border-amber-200 px-1.5 py-0.5 rounded text-[10px] font-semibold">थपिएको (डिफल्ट: <?php echo e($item->end_date?->format('Y-m-d')); ?>)</span>
                            <?php endif; ?>
                        </td>
                        ...
                        <td class="p-4 text-center">
                            <div class="flex flex-col items-center justify-center gap-2">
                                ...
                                <div class="flex items-center gap-1.5">
                                    <form action="<?php echo e(route('petrol.months.extend', $item->id)); ?>" method="POST" class="flex items-center gap-1">
                                        <?php echo csrf_field(); ?>
                                    ...
                                    <?php if($item->extended_end_date): ?>
                                        <form action="<?php echo e(route('petrol.months.clearExtension', $item->id)); ?>" method="POST">
                                            <?php echo csrf_field(); ?>
